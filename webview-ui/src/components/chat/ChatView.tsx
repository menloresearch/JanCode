...
const ChatViewComponent: React.ForwardRefRenderFunction<ChatViewRef, ChatViewProps> = (
	...
	const everVisibleMessagesTsRef = useRef<Set<number>>(new Set())
	...
	useEffect(() => {
		...
		if (prev) {
			...
			for (const tsKey in expandedRows) {
				if (Object.prototype.hasOwnProperty.call(expandedRows, tsKey)) {
					const ts = Number(tsKey)
					if (expandedRows[ts] && !(prev[ts] ?? false)) {
	...
	const visibleMessages = useMemo(() => {
		...
		// Update the set of ever-visible messages
		newVisibleMessages.forEach((msg) => everVisibleMessagesTsRef.current.add(msg.ts))
	...
	// const handleBrowserSessionItemToggleExpand = useCallback(
	// 	(messageTs: number) => {
